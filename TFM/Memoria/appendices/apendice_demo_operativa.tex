...
\figuraAnexo{fig_web_home.png}{Pantalla inicial de la consola web y \emph{sidebar} con las secciones operativas. Elaboración propia.}{fig:demo-web-home}
...
\figuraAnexo{fig_web_pantalla_infraestructura.png}{Pantalla \emph{Infraestructura}: comprobación de prerrequisitos y opciones de reset limpio. Elaboración propia.}{fig:demo-infra}
...
\figuraAnexo{fig_web_pantalla_ingesta.png}{Pantalla \emph{Ingesta}: servicios PostgreSQL y datasets \texttt{gold} descubiertos por OpenMetadata. Elaboración propia.}{fig:demo-ingesta}
...
\figuraAnexo{fig_web_pantalla_gobierno.png}{Pantalla \emph{Gobierno}: edición de la hoja \fqn{gold_governance.csv} con listas controladas para temática y categoría \ac{HVD}. Elaboración propia.}{fig:demo-gobierno}

\section{Aplicación del \emph{workflow}}\label{sec:demo-workflow}
...
\figuraAnexo{fig_web_pantalla_workflow.png}{Pantalla \emph{Workflow}: dry-run y aplicación con resumen de cambios. Elaboración propia.}{fig:demo-workflow}
...
\figuraAnexo{fig_web_pantalla_dcat.png}{Pantalla \emph{DCAT}: exportación del catálogo \ac{JSONLD} y previsualización. Elaboración propia.}{fig:demo-dcat}
...
\figuraAnexo{fig_web_pantalla_validacion.png}{Pantalla \emph{Validación}: ejecución de la suite reproducible y resumen consolidado. Elaboración propia.}{fig:demo-validacion}

Como complemento, la pantalla \emph{SHACL} muestra el manifiesto de las
\emph{shapes} \ac{DCATAPES} congeladas en \fqn{resources/shacl/manifest.json}:
origen, versión y \emph{commit}. Su función es evidenciar que la validación usa
recursos locales y no descarga \emph{shapes} en tiempo de ejecución.

\begingroup
\hbadness=3000
El informe \ac{SHACL}, en formato Turtle, incluye severidad
(\texttt{sh:Violation}, \texttt{sh:Warning}, \texttt{sh:Info}), nodo afectado y
mensaje. Se genera como artefacto \fqn{*_shacl_report.ttl} y se consulta desde
la pantalla \emph{Artefactos}. En la demostración no presenta violaciones
bloqueantes, en línea con la sección~\ref{cap:validacion}.
\par
\endgroup

\figuraAnexo{fig_web_pantalla_shacl.png}{Pantalla \emph{SHACL}: manifiesto de las \emph{shapes} DCAT-AP-ES congeladas (sin descarga en tiempo de ejecución). Elaboración propia.}{fig:demo-shacl}
...
\figuraAnexo{fig_web_pantalla_artefactos.png}{Pantalla \emph{Artefactos}: listado de evidencias con vista previa (\emph{Ver}) y apertura nativa (\emph{Abrir}) para HTML renderizado y PDF en el visor del navegador. Elaboración propia.}{fig:demo-artefactos}
...
\figuraAnexo{fig_web_pantalla_ejecuciones.png}{Pantalla \emph{Ejecuciones}: historial de \emph{jobs} con estado, duración, código de salida y acceso al log y artefactos. Elaboración propia.}{fig:demo-ejecuciones}
