...
En la pantalla \emph{Ingesta} se observa el resultado de la doble ingesta técnica. Aparecen los dos \texttt{DatabaseService} en \ac{OM}, \texttt{postgres\_demo\_service} y \texttt{postgres\_validation\_service}, cada uno con la misma estructura interna: la base de datos \texttt{opendata\_demo}, los esquemas \texttt{bronze}, \texttt{silver} y \texttt{gold}, y las tablas descubiertas en cada esquema. La demostración confirma que las dos tablas \texttt{gold} se han registrado en ambos servicios y, por tanto, generan cuatro datasets publicables identificados por su \ac{FQN}.
...
\paragraph{Idempotencia.} Repetir el botón \emph{Apply} una segunda vez es ilustrativo: el resumen indica \emph{0 cambios aplicados}, lo que evidencia visualmente la propiedad de idempotencia que el capítulo~\ref{cap:validacion} valida formalmente.
...
La pantalla \emph{Artefactos} expone todos los ficheros generados en \texttt{tmp\_pytest/} con vista previa estructurada para \ac{JSONLD}, Turtle, \ac{YAML}, \ac{CSV} y \ac{JSON}. El operario descarga directamente \texttt{dcat\_catalog.jsonld}, que es el artefacto final del recorrido. La pantalla \emph{Ejecuciones} (complementaria) lista cada \emph{job} con estado, duración, código de salida y enlace a sus artefactos asociados.

\figuraMemoria{fig_web_pantalla_artefactos.png}{Pantalla \emph{Artefactos}: previsualización y descarga del catálogo \ac{JSONLD} final. Elaboración propia.}{fig:demo-artefactos}
...
3 & Gobierno — editar + guardar & \fqn{gold_governance.csv}             & §~\ref{sec:implementacion-hoja} \\ \hline
...
6 & DCAT — exportar             & \texttt{dcat\_catalog.jsonld}         & §~\ref{sec:rdf-semantica} \\ \hline
7 & Validación — suite          & Informe consolidado \ac{JSON}         & cap.~\ref{cap:validacion} \\ \hline
8 & SHACL — manifiesto congelado & \texttt{manifest.json} (informe \texttt{.ttl} en Artefactos) & §~\ref{sec:shacl} \\ \hline
9 & Artefactos — descargar      & Catálogo \ac{RDF} entregable          & §~\ref{sec:demo-federacion} \\ \hline
\end{tabular}
