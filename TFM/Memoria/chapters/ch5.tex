...
Los requisitos funcionales describen las capacidades que la plataforma ofrece a sus actores, formuladas desde el punto de vista de quien las usa y no de la tecnología que las implementa. La tabla~\ref{tab:requisitos-funcionales} los enumera con identificador, descripción y criterio de aceptación verificable. Las propiedades transversales de calidad (reproducibilidad, idempotencia, portabilidad o trazabilidad) no son funcionalidades, sino requisitos no funcionales, y se recogen como tales en la sección~\ref{sec:requisitos-no-funcionales}.
...
RF-01 & Descubrir los activos técnicos de una fuente de datos. & Tras la ingesta, \ac{OM} registra el servicio, los esquemas, las tablas y las columnas de la fuente. \\ \hline
RF-02 & Gestionar activos procedentes de varios servicios u orígenes. & El catálogo diferencia los activos de dos \texttt{DatabaseService}; las tablas homónimas no se mezclan. \\ \hline
RF-03 & Identificar cada activo de forma unívoca. & Cada dataset se referencia por su \ac{FQN}; tablas con igual nombre en distinto servicio quedan separadas. \\ \hline
RF-04 & Curar los metadatos funcionales de cada dataset. & El responsable edita título, descripción, publicador, temática, categoría \ac{HVD} y \ac{URL} en la hoja \ac{CSV}. \\ \hline
RF-05 & Seleccionar qué datasets se publican en el catálogo. & Solo las filas con \texttt{publicar=si} aparecen en el catálogo exportado. \\ \hline
RF-06 & Aplicar el gobierno curado sobre OpenMetadata. & Las \emph{custom properties} y las etiquetas DCAT de cada tabla \texttt{gold} reflejan la hoja de gobierno. \\ \hline
RF-07 & Exportar el catálogo a \ac{JSONLD} conforme al perfil activo. & Se genera \texttt{dcat\_catalog.jsonld} con las clases \ac{DCAT} activas. \\ \hline
RF-08 & Validar el catálogo con \ac{SHACL} para el caso \ac{HVD}. & Se genera \texttt{dcat\_validation\_report.ttl} sin violaciones bloqueantes. \\ \hline
RF-09 & Permitir al operador ejecutar todo el flujo desde una consola web. & La \ac{UI} ejecuta cada etapa y muestra logs y artefactos. \\ \hline
\end{tabular}
...
\begin{itemize}
  ...
  \item \textbf{Simplicidad operativa}: una única capa Python concentra las reglas, mientras que la \ac{CLI}, los \emph{scripts} y la web son envoltorios.
...
Se priorizan la reproducibilidad y la idempotencia por encima de capacidades habituales en sistemas productivos, como la alta disponibilidad, el \ac{RBAC} avanzado o el escalado, que quedan fuera del alcance funcional declarado en la \hyperref[cap:introduccion]{introducción}.
...
El flujo implementado sigue una secuencia cerrada, resumida en el diagrama de secuencia de la figura~\ref{fig:secuencia} y agrupada por fases en la figura~\ref{fig:flujo-operativo}. La primera fase despliega la infraestructura e ingiere la fuente PostgreSQL dos veces en OpenMetadata, bajo dos servicios diferenciados, y prepara las etiquetas y las \emph{custom properties} \ac{DCAT}. La segunda fase ejecuta el \emph{workflow}, que refresca la hoja de gobierno, permite revisar los metadatos funcionales y aplica los cambios sobre OpenMetadata de forma idempotente. La tercera fase exporta el catálogo en \ac{JSONLD} y ejecuta la validación \ac{SHACL}. La transformación conceptual de los metadatos, desde los activos técnicos hasta la salida validada, se representa más adelante en la figura~\ref{fig:pipeline-metadatos}.

\figuraMemoria[0.6\linewidth]{fig_flujo_operativo_validacion.png}{Flujo operativo del caso de uso de validación, agrupado por fases. Elaboración propia.}{fig:flujo-operativo}
...
La doble ingesta usa la misma base de referencia, pero crea dos \texttt{DatabaseService} en OpenMetadata: \texttt{postgres\_demo\_service} y \texttt{postgres\_validation\_service}. El objetivo no es duplicar datos por volumen, sino comprobar que el gobierno funciona cuando existen tablas con el mismo nombre en servicios distintos. Por eso la hoja funcional conserva \texttt{schema\_name} y \texttt{table\_name} para la lectura humana, pero usa \texttt{table\_fqn} como clave de identificación: al incluir el servicio, la base de datos y el esquema, es el único valor que distingue dos tablas homónimas de servicios distintos y evita que el gobierno las fusione.
...
La curación funcional se concentra en \fqn{tfm_ingestor/config/gold_governance.csv}. Esta hoja permite editar los metadatos que varían por dataset: decisión de publicación, título, descripción, publicador, temática DCAT, categoría HVD y URL de acceso. Los campos técnicos (esquema, tabla y \ac{FQN}) no deben editarse manualmente salvo regeneración controlada del caso de uso. Las columnas de la hoja se resumen en la tabla~\ref{tab:hoja-gobierno-columnas} del Anexo~\ref{ap:capturas}, y el detalle completo con criterios de validación, en el Anexo~\ref{ap:ingesta-gobierno}.
...
Los metadatos globales del catálogo se derivan de \texttt{governance\_defaults.yaml}: nombre del catálogo, descripción, publicador, URI de organismo, página principal, idioma, licencia, legislación HVD, contacto y bases de URLs. Estos valores operan como \emph{defaults} a dos niveles: describen el \ident{dcat:Catalog} y, además, sirven de valor por defecto heredable por cada \ident{dcat:Dataset}. Esta separación evita repetir información común en cada dataset y reduce errores de consistencia. Cuando un dataset concreto requiere un publicador, una licencia, un idioma o un contacto propios, algo habitual cuando coexisten varios organismos o responsables, la hoja funcional puede sobrescribir el \emph{default} global a nivel de fila sin modificar el núcleo. El perfil operativo que conecta esos defaults con la hoja \ac{CSV} y fija el caso \ac{HVD} como modo de validación se recoge en el listado~\ref{code:operational-profile-yaml} del Anexo~\ref{ap:capturas}.

\subsubsection{Núcleo único de lógica}

Toda la lógica de gobierno vive en Python, y la \ac{CLI}, los \emph{scripts} y la web invocan ese núcleo sin reimplementar reglas, lo que mantiene la solución probable con \texttt{pytest}. El punto de entrada canónico es una \texttt{dataclass} inmutable (\texttt{frozen=True}) que fija todos los parámetros del \emph{workflow} antes de ejecutarlo (su firma se detalla en la \hyperref[cap:implementacion]{implementación}). El término \emph{dry-run} («ejecución en seco») designa una ejecución que calcula y muestra el plan de cambios sin aplicarlos sobre \ac{OM}; se conserva en inglés por ser el nombre literal de la opción de la \ac{CLI} (\texttt{-{}-dry-run}).
...
\subsection{Lógica central en Python: paquete \texttt{om\_dcat\_sync}}\label{sec:implementacion-nucleo}

El paquete \texttt{tfm\_ingestor} (alias importable \texttt{om\_dcat\_sync}) concentra toda la lógica de gobierno, exportación y validación. Está estructurado en módulos con responsabilidades acotadas, conforme al principio de lógica única declarado en la sección~\ref{cap:arquitectura}. La tabla~\ref{tab:modulos-python} lista los módulos principales y la figura~\ref{fig:modulos-python} del Anexo~\ref{ap:capturas} resume su composición.
...
\texttt{workflow\_service.py} & Orquestador canónico del \emph{workflow} de extremo a extremo. \\ \hline
...
\texttt{config.py} & Carga \emph{defaults} \ac{YAML} y reglas de mapeo. \\ \hline
\fqn{operational_profile.py} & Une \emph{defaults}, reglas y caso de validación. \\ \hline
...
\subsection{Hoja funcional de gobierno y \emph{defaults} globales}\label{sec:implementacion-hoja}

La curación funcional se concentra en \fqn{tfm_ingestor/config/gold_governance.csv}, cuya estructura columnar se describe en la tabla~\ref{tab:hoja-gobierno-columnas} y se detalla con criterios de validación en el Anexo~\ref{ap:ingesta-gobierno}. El uso de \ac{CSV} con separador \texttt{;} no es accidental: facilita la edición en hojas de cálculo locales sin Excel y, sobre todo, hace que el diff en Git sea legible para revisión editorial. Los defaults globales viven en \fqn{governance_defaults.yaml}, que centraliza publicador, homepage, idioma, licencia, contacto y la legislación \ac{HVD} aplicable. La pantalla \emph{Gobierno} de la consola web (figura~\ref{fig:web-pantalla-gobierno}, Anexo~\ref{ap:capturas}) actúa sobre la misma hoja y aplica las mismas reglas de validación que el \ac{CLI}; su uso real se narra en la sección~\ref{sec:demo-gobierno} del Anexo~\ref{ap:demo-operativa}.
...
\begin{lstlisting}[language=PowerShell, caption={\emph{Workflow} canónico de extremo a extremo: sincroniza gobierno, exporta DCAT-AP-ES y valida SHACL HVD.}, label=code:workflow-run, captionpos=b]
...
Segunda ejecución & 0 cambios aplicados & El \emph{workflow} es idempotente. \\ \hline
\end{tabular}
...
\begin{itemize}
  ...
  \item plan de \emph{workflow} en JSON;
...
La solución aborda el problema planteado al conectar los activos gestionados en OpenMetadata con una salida interoperable conforme a DCAT-AP-ES y validable mediante \ac{SHACL}. La plataforma no se limita a definir un modelo de correspondencia, sino que implementa un flujo ejecutable que descubre activos técnicos, incorpora metadatos funcionales, genera un catálogo \ac{JSONLD} y verifica su conformidad. El Anexo~\ref{ap:demo-operativa} ilustra este proceso de extremo a extremo, desde la operación de la consola web hasta la obtención de un catálogo \ac{RDF} validado.

La utilización de una fuente técnica reproducible basada en PostgreSQL permite demostrar el gobierno de metadatos desde los activos de origen, y no únicamente sobre metadatos ya publicados. Este enfoque resulta más adecuado para evidenciar el proceso completo de catalogación, enriquecimiento, exportación y validación que constituye el objetivo principal del trabajo.
...
\begin{itemize}
  ...
  \item núcleo Python único para CLI, \emph{scripts} y web;
...
\begin{itemize}
  ...
  \item \textbf{Validez externa}: el caso de uso emplea datos sintéticos y un número reducido de datasets de una única fuente PostgreSQL, por lo que la generalización a catálogos institucionales a gran escala queda fuera de lo demostrado. La arquitectura está diseñada para escalar a más tablas \texttt{gold} y a otras fuentes con la misma hoja funcional y el mismo \emph{workflow}.
...
Análisis DCAT-AP-ES & \hyperref[cap:estado-arte]{Estado del arte} & \texttt{docs/dcat\_mapping.md}, bibliografía oficial \\ \hline
Mapeo OpenMetadata & \hyperref[cap:arquitectura]{Arquitectura} e \hyperref[cap:implementacion]{implementación} & \texttt{dcat\_export.py}, \texttt{governance\_service.py} \\ \hline
Configuración de gobierno & \hyperref[cap:implementacion]{Implementación} & \texttt{gold\_governance.csv}, \texttt{governance\_defaults.yaml} \\ \hline
Doble ingesta PostgreSQL & \hyperref[cap:implementacion]{Implementación} y \hyperref[cap:validacion]{validación} & \texttt{ingest\_postgres\_double.ps1}, \texttt{gold\_governance.csv} \\ \hline
\emph{Workflow} reproducible & \hyperref[cap:implementacion]{Implementación} y \hyperref[cap:validacion]{validación} & \texttt{workflow\_service.py}, \texttt{run\_validation\_suite.ps1} \\ \hline
Exportación JSON-LD & \hyperref[cap:validacion]{Validación} & \texttt{dcat\_export.py}, artefactos \texttt{*.jsonld} \\ \hline
Validación SHACL & \hyperref[cap:validacion]{Validación} & \texttt{shacl\_validation.py}, \emph{shapes} vendorizadas \\ \hline
Consola operativa & \hyperref[cap:implementacion]{Implementación} & \texttt{web/}, \texttt{operations.ts}, \texttt{jobRunner.ts} \\ \hline
\end{tabular}
