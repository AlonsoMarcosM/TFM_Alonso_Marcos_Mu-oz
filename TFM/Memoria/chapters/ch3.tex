...
\section{Reproducibilidad, idempotencia e \emph{Infrastructure as Code}}\label{sec:reproducibilidad}

La reproducibilidad es un requisito transversal de la ingeniería de plataformas: un entorno debe poder reconstruirse de forma determinista a partir de su descripción declarativa, sin depender del estado mutable de la máquina que lo levantó~\cite{kubernetesConcepts}. La combinación de Docker, Kind, Helm y manifiestos versionados implementa este principio, al describir el clúster como datos (\ac{YAML}) y no como pasos manuales. Su complemento operativo es la idempotencia: una operación es idempotente cuando aplicarla varias veces produce el mismo resultado que aplicarla una sola vez. En gobierno de metadatos es crítica, porque un \emph{workflow} no idempotente duplicaría etiquetas o sobrescribiría campos curados. La plataforma se apoya en que los \texttt{PATCH} de la \ac{API} \ac{REST} de \ac{OM}~\cite{openmetadataDocs} son idempotentes para garantizar que la segunda ejecución no aplique cambios, propiedad verificada en la sección~\ref{cap:validacion}. Las pruebas completan el marco, con pytest~\cite{pytestDocs} sobre el núcleo Python y Vitest~\cite{vitestDocs} sobre la consola web.
